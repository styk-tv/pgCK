...
This is the **Concept Kernel Protocol** (CKP, v3.12 wave). The root ontology ships **in this repository** at [`ontology/v3.12/core.ttl`](ontology/v3.12/core.ttl), pinned by a per-revision `.sha256` sidecar beside it — so the law a release enforces is a file you can hash yourself:

```sh
cd ontology/v3.12 && shasum -a 256 -c core.ttl.wave-3.12-pass-3.sha256   # OK
```

> **The wire IRIs are the `v3.11` namespace**, and deliberately so — the v3.12 wave revises the law without renaming it, so a client's IRIs keep working across the wave. Always write full IRIs; the namespace is `https://conceptkernel.org/ontology/v3.11/core#`.

Reference authority: [conceptkernel.org](https://conceptkernel.org).
...
And here's what your app (or browser, or agent) actually writes — cklib over NATS-WSS, nothing else. Types are the root ontology's own (always full IRIs on the wire):
...
```sh
gh attestation verify oci://ghcr.io/styk-tv/pgck:0.4.109-pg18-amd64 --repo styk-tv/pgCK   # exit 0
```
...
- **Self-changing types** — `propose → vote → apply` mutates the kernel shape by quorum; the next seal is bound by it, with a full proof chain from proposal to applied epoch. Quorum is a count of *distinct accountable parties*; a `shared` kernel refuses a quorum of one, and says so.
- **Ownership is immovable, not merely checked** — `ckp:ownedBy` cannot be patched by anyone, owner included, and neither can the kernel's quorum policy. There is deliberately **no ownership-transfer verb yet**: a lawful hand-over must itself be governed, and until it exists, ownership simply does not move.
- **Authority narrows, never widens** — in-kernel roles can only *restrict* what a participant may do per action; they can never grant beyond the substrate floor, and an owner cannot be narrowed out of their own kernel.
- **Acting on behalf of someone is a server-derived fact** — an agent's seal carries `onBehalfOf` stamped from the verified connection; a client that puts it in the payload has it **stripped**, not honoured.
- **Rules can be scoped to a named set** — a law bound to a `ckp:Scope` binds its members and no one else, so a kernel can hold several regimes at once without them leaking into each other.
- **Every refusal is typed** — no untyped error sites remain on the governed path; each refusal returns a registered code that matches the wire contract, so a caller can branch on *why* rather than parse prose.
- **Scheduled work enqueues, never executes** — periodic ticks draft and queue; they seal, vote and apply nothing. Draining is bounded, attempt-counted, fair across kernels, and a repeatedly failing job parks instead of spinning.
...
- **Module adoption is governed and pinned** — a module reaches a kernel's enforcement
  ...
  the pin ledger out of the box. Proximity is never adoption: a module sitting in the same
  database governs nothing until a kernel adopts it by agreement.
- **Being named in a scope is not yet consent** — a `ckp:Scope` binds who it lists, but
  listing someone is not an act they agreed to. Consent is a known, recorded gap.
- **Upper-ontology grounding** (BFO and friends) and **cross-kernel federation** are the
  *trajectory* — captured as direction, built only when a real consumer needs them, never
  speculatively.
- `ed25519` will replace the shipped `hmac+sha256` proof method. Note what today's flag does
  and does not say: `verified: true` is the **hash chain**, not conformance — the shape a fact
  was judged under is a separate stamp (`conformsToShape`), and a fact can be sealed and
  chained while no shape selected it. Read both.
...
> **Name your kernel — an unnamed one now refuses.** When `ckp.project` is unset the substrate does not guess: sealing is refused with *"no kernel named … the law is in force and readable; sealing refuses until a kernel is named, because a fact must say whose law governs it."* Earlier releases silently fell back to a default project, which could split facts across two kernels without saying so. Set `ckp.project` (session or `postgresql.conf`) to the same kernel `pgck.kernels` serves; the pre-composed bundles already pin both.
...
The local loop is **Docker** (any context — set `DOCKER_CONTEXT` if you use a non-default one). `just` builds the Linux artifacts into `compose/extensions/` and runs the isolated stack.
...
just smoke-s4        # warm governed suite (s4…s89)
just smoke-s34       # fresh-install gate (CREATE EXTENSION from zero)
just psql            # psql into the compose postgres — operator/debug only
```

Both suites must be green before a release. Every gate under [`sql/test/`](sql/test/) ships with the **negative control** that proves it can fail — a check that cannot fail the thing it claims is not a check. A browser-facing NATS-WSS stack (`just nats-wss-up`, `just smoke-nats-wss`) is available for end-to-end WSS testing.
