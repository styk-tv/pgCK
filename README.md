...
> **Set your kernel explicitly on both planes.** Through ≤ 0.4.78 the SQL/operator path falls back to the project `demo` when `ckp.project` is unset, while the wire serves whatever `pgck.kernels` names — so mixing `psql` writes and wire writes can split your facts across two kernels without saying so. Set `ckp.project` (session or `postgresql.conf`) to the same kernel `pgck.kernels` serves. The pre-composed bundles already pin both.
