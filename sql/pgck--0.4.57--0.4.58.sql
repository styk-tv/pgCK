-- pgck 0.4.58 — the oracle: the pass boundary is a query, never a memory.
--
-- wave.oracle answers, from sealed facts only: what belongs to THIS pass
-- (everything stamped with its number, any of the six *AtPass/forPass stamps),
-- what carries to the NEXT (open findings, pending unretired proposals, and the
-- inbox — Operations addressed to this component), and the health signals a
-- third party can recompute (unjudged, preEnforcement, anonymousSeals, open/
-- pending counts). Never aggregated into one boolean; the fence — sealed but
-- judged by nothing — is a number readable on the first call (v3.12 §5, first
-- built piece; retires into governed SPARQL reads when stamp projection lands).
--
