-- pgck 0.4.52 — the checker surface becomes verbs on this kernel.
--
-- PASS-30 answered four questions with hand-written psql probes: is this type
-- admitted and by WHICH graph · which admitted classes does no shape target ·
-- what does this type declare · what does this transport segment resolve to and
-- why. Every answer mattered and not one was re-runnable by the party who needed
-- it. A check that lives in a scratch directory is a second enforcement surface
-- with worse provenance than the first — which is the defect class this whole
-- record exists to end, committed by the component that keeps filing it.
--
--   surface.typecheck {type}   admitted? shaped? via which graph, at which digest
--   surface.unshaped           classes the surface declares that no shape targets
--   surface.declared {type}    the property contract, learnable WITHOUT writing
--   project.resolve {segment}  which kernel this name means, and under which clause
--
-- Each calls the SAME internal the gate calls — _type_admitted, _propmap,
-- _project, _composed_shapes — never a parallel implementation, because a probe
-- that re-implements the gate tests the probe.
--
-- Seeded into the registry here so they DISPATCH; sealed as ckp:Affordance
-- through propose -> vote -> apply so they are DECLARED. Both halves in one
-- version, or this grows the 1-declared/28-unsealed gap it exists to close.
--
...
BEGIN
  v_comp := ckp._composed_shapes(v_proj);
  v_ci   := pgrdf.graph_iri(v_comp);
  -- Declarative on purpose: this is SPARQL over the composed surface, not a scan
  -- of anything pgCK keeps on the side. The vacuity it finds is the SAME vacuity
  -- ckp.seal would produce, because both read this graph.
  SELECT COALESCE(jsonb_agg(j->>'c' ORDER BY j->>'c'), '[]'::jsonb) INTO v_rows
  FROM pgrdf.sparql(format($q$
    PREFIX sh:   <http://www.w3.org/ns/shacl#>
    PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
    PREFIX owl:  <http://www.w3.org/2002/07/owl#>
    SELECT DISTINCT ?c WHERE { GRAPH <%1$s> {
...
DECLARE
  ...
  v_kid   text; v_hits text[]; v_ask text; v_out text; v_why text;
BEGIN
  v_kid := 'urn:ckp:'||v_seg||'/kernel';
  SELECT array_agg(DISTINCT i.body->>'@id') INTO v_hits
    FROM ckp.instances i
   WHERE i.body->>'type' = N||'Kernel' AND lower(i.body->>'@id') = lower(v_kid);
  BEGIN
    -- ask the REAL resolver, so this can never drift from what a seal would do
    SELECT set_config('ckp.project', v_seg, true) INTO v_out;
    v_out := ckp._project();
    v_why := CASE
      WHEN v_out = v_seg AND v_seg ~ CANON AND v_hits @> ARRAY[v_kid] THEN 'clause 1 — canonical, and it carries its own sealed kernel'
      WHEN v_out <> v_seg THEN 'clause 2 — resolved onto the one CANONICAL sealed kernel that answers to this name'
      WHEN v_seg ~ CANON THEN 'clause 4 — canonical with no sealed kernel yet; the germination path stays open'
      ELSE 'clause 5 — grandfathered: no sealed kernel, but a kernel graph stands behind it (germinated before the canonical rule)'
    END;
  EXCEPTION WHEN OTHERS THEN
    RETURN jsonb_build_object('ok', true, 'segment', v_seg, 'resolves', null,
      'refused', true, 'reason', SQLERRM,
      'sealedKernelsMatching', COALESCE(to_jsonb(v_hits), '[]'::jsonb));
  END;
  RETURN jsonb_build_object('ok', true, 'segment', v_seg, 'resolves', v_out,
    'canonical', (v_seg ~ CANON), 'clause', v_why,
    'sealedKernelsMatching', COALESCE(to_jsonb(v_hits), '[]'::jsonb),
    'note', 'one kernel has one spelling. The answer comes from ckp._project itself, never a copy of its rules.');
END;
$function$
;
...
-- The registry seeds. ON CONFLICT DO NOTHING keeps this idempotent, and the
-- routing plane is 'instance' (the built-in CASE), NOT the root's semantic plane
-- — the column conflates the two and that is recorded as a #56-adjacent finding.
