-- pgck--0.4.79--0.4.80.sql — A LEDGER KEY OF ONE'S OWN.
--
-- MEASURED 2026-08-21 on the latest published bundle: every deployment signed
-- its proof chain with the literal 'pgck-localhost' — this project's own dev
-- bench name, hardcoded in ckp.dispatch since 2026-06-04 (7e94893) and shipped
-- in every release since. `verified: true` therefore meant "hashes correctly
-- under a key everyone knows", which is an integrity check against corruption,
-- not against a party. Not remotely exploitable (the role floor guards the seal
-- path) but weaker than SECURITY §2.5's "verified at time, BY THIS STORE"
-- implies — because by default there was no key belonging to any store.
--
-- Third instance this week of one mechanism: a value correct as a local
-- convenience, placed in a slot no gate reads, carried forward by everyone
-- doing the right thing. 'demo' in the naming plane (0.4.79), 'pgCK' in the
-- transport plane (0.4.78), and this one in the EVIDENCE plane.
--
-- The rule that predicts the rest of them: absence must REFUSE, never
-- substitute. And minting is not defaulting — a default hands every install the
-- same answer; a mint hands each one its own.
--
-- ============================================================================
-- THE UPGRADE PATH: NOTHING IS SUBSTITUTED. NOT EVEN THE OLD VALUE.
-- ============================================================================
-- A first draft of this migration inserted the literal 'pgck-localhost' when the
-- row was absent, to keep existing stores' proofs verifying. That is
-- substitution-on-absence with a baked-in constant — the exact defect this
-- release exists to delete, re-added by its own author while deleting it.
-- WITHDRAWN.
--
-- So: this migration writes NO key. A store upgrading from <= 0.4.79 has no
-- config row (that slot was never populated), so ckp.seal's existing refusal
-- becomes reachable and sealing stops until an operator names a key. That
-- refusal IS the notification, and it names the remedy.
--
-- The consequence, stated rather than smoothed over: facts sealed before this
-- upgrade were signed with a constant shipped in every release. Under a new key
-- they no longer verify — which does not destroy evidence so much as REVEAL what
-- that evidence was always worth. An operator who needs the old chain to keep
-- verifying can set the previous value deliberately (the CHANGELOG names it);
-- that is their decision, on their record, not ours baked into a migration.

-- The key must travel with the facts it signs. ckp.config was never dump-flagged
-- while ckp.ledger was, so a restore would bring back every sealed fact and lose
-- the key — masked until now only because the key was a shared constant.
DO $dump_0480$
BEGIN
  PERFORM pg_catalog.pg_extension_config_dump('ckp.config', '');
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'pgck 0.4.80: pg_extension_config_dump(ckp.config) skipped (%)', SQLERRM;
END
$dump_0480$;

-- ckp.dispatch no longer manufactures a key: it reads the GUC, then the config
-- row, and ckp.seal's existing "no identity key configured" refusal becomes
...
-- P9 (warm path): the compute queue gets its retry bound. See the completeness
-- block for the reasoning — a live drain with no bound presents as a working
-- loop the first time a job fails.
