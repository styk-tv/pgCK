-- pgck--0.4.82--0.4.83.sql — THE ENVELOPE IS ONE LAW (HANDOVER B7; cklib PASS-2
-- ISSUE-6 + ISSUE-7's substrate half; suite cases 29/30).
--
-- Measured defect: the SAME refusal class shipped {refused:true} from one
-- construction site (instance.link) and bare {ok:false,error,hint} from another
-- (instance.create) — 101 hand-built ok:false objects, no single definition of
-- what a refusal looks like on the wire, so no consumer could structurally tell
-- a refusal from a fault (cklib's outcomeOf correctly refused to manufacture
-- the flag from prose).
--
-- The fix is one law at the door, never a 101-site patch sweep:
--   (1) ckp.refusal_registry — the DECLARED closed set of refusal codes, each
--       with a typed sqlstate class (pgck's own E0) and, where the caller's KEY
--       was the mistake, the teaching hint (invalid_about now teaches
--       {about: <proposal_iri>}; unknown_proposal teaches the @id form).
--   (2) ckp._refusal_envelope — the normalizer (ckp._envelope was TAKEN:
--       the instance read-envelope builder — one name, one meaning), applied ONCE on the way out of the
--       door: code in registry ⇒ refused:true + sqlstate (+hint if owed and
--       absent). Idempotent over sites that already flag; an error NOT in the
--       registry is fault-shaped and stays unflagged — deliberately (the
--       SQLERRM catch-alls and internal *_failed wrappers are faults).
--   (3) ckp.dispatch(text,jsonb) becomes a thin wrapper over the route body
--       (renamed ckp._dispatch_route, internal, no role-floor grant — the
--       completeness floor grants by proname 'dispatch' and is unaffected).
--   (4) surface.refusals — the registry is learnable through the door (a
--       check that is not a verb does not exist).
--
-- Both roads in one act: the baseline carries the identical statements for the
-- install path; this migration was GENERATED from those baseline bytes.
...
-- The routed half of surface.refusals for the warm road (the baseline's seed
-- INSERT only runs at install). Declared half stays with #56's train.
