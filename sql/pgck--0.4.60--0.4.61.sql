-- pgck 0.4.61 — the adoption pin is consulted: graph drift is detectable.
--
-- ck-dev's controlled test (their defect-1786649168553127000): a module composed
-- under a sourceDigest of sixty-four zeros — the pin was judged by AdoptionShape
-- and consulted by NOTHING. The honest split of the fix: a FILE-BYTE digest
-- cannot be recomputed from triples, so sourceDigest verification belongs to
-- whatever loads the file; the SUBSTRATE's half is the graph pin — the module
-- graph's canonical digest recorded at first composition (ckp.adoption_pins),
-- and adoption.check reporting drift per module ever after. Reports, never
-- gates (B4): a false drift-positive in the composition hot path would refuse
-- every seal for the project, and digests over materialized graphs can shift
-- under engine upgrades. Detection first; refusal only after stability is
-- measured across an engine upgrade. Trust-on-first-sight, named as such.
--
