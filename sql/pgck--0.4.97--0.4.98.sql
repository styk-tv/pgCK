-- pgck 0.4.97 -> 0.4.98
--
-- C-1 / L-8 — projectKind FINALLY MEANS SOMETHING.
--
...
--
-- WHAT THIS IS FOR, corrected by measurement rather than assumed. The obvious
-- framing - "stop kernels self-approving" - is wrong on the door where it
-- matters: ckdev has 16 proposals and NONE declare quorum 1, because CK.Lib.Js's
-- handshake #2 corrected the practice with no gate at all. pgck still runs 17 of
-- 22 at quorum 1 and some of that is legitimate on a destructive bench. So this
-- is not stopping current abuse. It is PREVENTING REGRESSION: practice that
-- holds only by convention is one hurried session from lapsing, and the ledger
-- cannot tell a principled quorum-2 from a lucky one.
--
-- Three properties, and two are the safety:
--   shared     -> floor 2. Declaring you need a partner binds you.
--   personal   -> floor 1. Quorum-of-one is its DECLARED meaning; refusing it
--                 would make this a wall and break every legitimate solo project.
--   undeclared -> floor 1, imposing NOTHING. A project sealed before projectKind
--                 was required has not declared a mating type, and inventing one
--                 is the substrate choosing a reproductive strategy nobody asked
--                 for - the defect 0.4.81 fixed by defaulting to NULL rather
--                 than 'personal'.
--
-- The floor binds at PROPOSE and again at APPLY, via GREATEST rather than
-- replacement, so a proposal sealed at quorum 1 before its project declared
-- shared cannot still be self-applied, and one that asked for MORE keeps it.
--
-- The refusal names the DECLARATION, not a policy: it quotes the project's own
-- projectKind and its meaning, and offers the alternative (govern the project to
-- personal first - a decision with a record, which is the point).
--
-- Measured end-to-end before shipping: with a shared Project, floor 2, quorum 1
-- REFUSED with the clause, quorum 2 ACCEPTED. The second half is the control -
-- a floor that refuses everything is a wall.
