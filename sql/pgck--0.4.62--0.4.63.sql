-- pgck 0.4.63 — wave.oracle is wave.signals.
--
-- Renamed before any tagged release carried the old name (0.4.58 lived only on
-- a branch). Two reasons, pointing the same way: "Oracle" is Oracle Corp's
-- strongest trademark in exactly this product class, and the verb never
-- prophesied anything — it DERIVES signals from sealed facts, which is what
-- v3.12 §5 calls it anyway. The accurate name and the cautious name coincide.
-- wave.oracle remains as a one-version routing alias that answers and names
-- its replacement; it is removed next release.
--
...
-- the old function name retires with the rename; the alias routes to wave_signals
DROP FUNCTION IF EXISTS ckp.wave_oracle(jsonb);
