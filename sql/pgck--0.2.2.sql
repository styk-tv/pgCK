...
-- ckp.outbox table + index + the ckp_ledger_after_insert trigger are created
-- inside ckp.bootstrap_kernel() (above), NOT here, because they depend on
-- ckp.ledger which bootstrap_kernel creates. Defining them as install-time
-- top-level DDL broke fresh CREATE EXTENSION (ckp.ledger does not exist yet).
-- The trigger FUNCTION is still defined top-level below (its body is not
-- resolved until the trigger fires at runtime).
...
-- The ckp_ledger_after_insert trigger is created inside ckp.bootstrap_kernel()
-- (it binds to ckp.ledger, created there). See the bootstrap_kernel body above.
