-- pgck 0.4.47 — ship the dispatch overload that actually routes
--
-- 0.4.46 fixed ckp.dispatch to resolve the CALLER's affordances and then
-- shipped the wrong function. There are two overloads:
--     dispatch(p_verb, p_kernel_urn, p_payload, p_identity)   -- thin wrapper
--     dispatch(p_verb, p_payload)                             -- the routing logic
-- The migration generator matched the name prefix and took the FIRST, so the
-- wrapper was re-created and the routing function was left as it was. The
-- baseline carried the fix; the upgrade route did not.
--
-- MEASURED on the bench after 0.4.46 (reported by pgCK.MCP, re-run here):
--     ckp._project()                                 = pgck-mcp
--     registry_lookup('pgck-mcp','surface.targets')  = plane=query, epoch=2
--     dispatch('surface.targets')                    = unknown_affordance
-- The GUC was right, the row was right, the lookup was right in isolation --
-- and the deployed dispatch still read registry_lookup('pgck', ...). So a verb
-- a kernel registered, voted through and applied could not be called by that
-- kernel. pgCK.MCP read the cause as ckp._project() falling back to 'demo';
-- it does not -- it returned pgck-mcp correctly. The fault was the overload.
--
-- WHY NO GATE CAUGHT IT: smoke-s4 and smoke-s34 both install from the BASELINE,
-- which had the fix. Nothing exercises the migration chain, so a migration that
-- ships the wrong function passes every test. The bench is the only place the
-- upgrade route runs, and it runs unverified.
