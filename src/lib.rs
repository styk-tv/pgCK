...
// v3.11 BASELINE INSTALL (pgCK#31). The 31-chunk migration chain is retired from
// the build and replaced by one install, generated by CK-org from this repo's own
// chain-built reference and proven against the obligation pgCK set in PASS-13:
// a fresh install produces an IDENTICAL routine set — 80 routines, identical
// signatures, none gained, none lost — with exactly 11 body digests differing, and
// those 11 are exactly the 11 that carried the v3.8 namespace literal.
//
// The chain was never an upgrade path. Two of its steps (pgck--0.3.1--0.3.2.sql,
// pgck--0.4.0--0.4.1.sql) are ABSENT FROM THE REPO, so it could not replay from any
// earlier version and nothing reported that. It is retained on disk as history and
// is no longer built.
//
// ATOMIC WITH ADOPTION, and this is not optional: the baseline embeds the v3.11
// namespace in 14 places. Wiring it against the v3.8 ontology would make every
// ledger and proof entry v3.11-typed against v3.8 shapes — targeted by nothing,
// conforms=true, silent. Measured both directions in PASS-10 and PASS-17. So
// ontology/core.ttl moves to the v3.11 root (e5f7d1e5…) in this same change.
extension_sql_file!("../sql/pgck-baseline.sql", name = "pgck_v311_baseline");
